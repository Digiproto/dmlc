...
void gen_skyeye_cfile(device_t *dev, const char *out) {
    const char *dev_name = dev->obj.name;
    char tmp[1024];
    FILE *f;

    snprintf(tmp, 1024, "%s/%s_skyeye.c", out, dev_name);
    f = fopen(tmp, "w");
    if(f) {
        setbuf(f, NULL);
    ...
    snprintf(tmp, 1024, "%s/%s_skyeye_protos.c", out, dev_name);
    f = fopen(tmp, "w");
    if(f) {
