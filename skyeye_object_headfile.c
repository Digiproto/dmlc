...
static const char *headfile_dir = "simics";
static void gen_object_struct(device_t *dev, FILE *f) {
    ...
    fprintf(f, "#ifndef %s_SKYEYE_STRUCT_H\n", cap_name);
    fprintf(f, "#define %s_SKYEYE_STRUCT_H\n", cap_name);
...
static void gen_object_headfile(device_t *dev, FILE *f) {
    ...
    fprintf(f, "/* %s */\n", asctime(localtime(&timep)));
    fprintf(f, "\n");
    fprintf(f, "#ifndef %s_SKYEYE_H\n", captical_name);
    fprintf(f, "#define %s_SKYEYE_H\n", captical_name);
    fprintf(f, "\n");
    fprintf(f, "#include \"%s/base_types.h\"\n", headfile_dir);
    fprintf(f, "#include \"%s/generic_transaction.h\"\n", headfile_dir);
    fprintf(f, "#include \"%s_skyeye_struct.h\"\n", dev_name);
    fprintf(f, "\n");
...
void gen_skyeye_headerfiles(device_t *dev, const char *out_dir) {
    ...
    snprintf(tmp, 1024, "%s/%s_skyeye.h", out_dir, dev_name);
    ...
    snprintf(tmp, 1024, "%s/%s_skyeye_struct.h", out_dir, dev_name);
    f = fopen(tmp, "w");
    if(f) {
